...
private[snunit] object ZioAsyncServerBuilder {
  ...
  def build: IO[Unit] = IO {
    ...
    init.callbacks.add_port = ZioAsyncServerBuilder.add_port
    init.callbacks.remove_port = ZioAsyncServerBuilder.remove_port
    init.callbacks.quit = ZioAsyncServerBuilder.quit
    nxt_unit_init(init)
  }.flatMap(ctx =>
